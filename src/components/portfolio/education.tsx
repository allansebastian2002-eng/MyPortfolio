          ...
          <div className="border-l border-border pl-6 sm:pl-8 space-y-10">
            {EDU.map((item) => (
              <div key={item.institution} className="relative">
                ...
                <span className="absolute -left-[31px] sm:-left-[39px] top-1.5 w-2 h-2 rounded-full bg-accent ring-4 ring-background" />
                ...
                {item.detail && (
                  <p className="mt-3 text-sm text-foreground/70 leading-relaxed max-w-xl font-light">
                    {item.detail}
                  </p>
                )}
              </div>
